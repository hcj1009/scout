...
gsl::span<gsl::byte const> extract(gsl::span<T, Dimensions...> dest,
		gsl::span<gsl::byte const> src) {
...
template<typename T, std::ptrdiff_t ... Dimensions>
gsl::span<gsl::byte> flatten(gsl::span<gsl::byte> dest,
		gsl::span<T const, Dimensions...> src) {
	static_assert(std::is_trivial<std::decay_t<T>>::value, "Target type must be a trivial type");
...
std::vector<gsl::byte> message_dht_blob_write(
		gsl::span<gsl::byte const> msg_data, chash_span next_msg_hash);
std::vector<gsl::byte> message_dht_blob_read(
		gsl::span<gsl::byte const> dht_blob, hash& next_msg_hash);

void log_debug(char const* fmt, ...);
void log_error(char const* fmt, ...);

// crypto helper functions:
std::vector<char> decrypt_buffer(std::vector<char> buffer,
		secret_key_span secret);
std::vector<char> encrypt_buffer(std::vector<char> buffer,
		secret_key_span secret, const unsigned char* nonce_in = nullptr);
