...
struct udp_socket: public std::enable_shared_from_this<udp_socket> {
	...
	udp_socket(udp_socket&& o) :
			m_socket(std::move(o.m_socket)), m_sender(std::move(o.m_sender)), m_handler(
					std::move(o.m_handler)), m_abort(std::move(o.m_abort)) {
		std::memcpy(m_receive_buffer, o.m_receive_buffer,
	...
	void start(handler_t const& h, udp::endpoint const& ep, error_code& ec) {
		...
		m_socket.async_receive_from(
				buffer(m_receive_buffer, sizeof(m_receive_buffer)), m_sender,
				std::bind(&udp_socket::on_receive, shared_from_this(), _1, _2));
	}

	int send_to(char const* buf, int len, udp::endpoint const& ep,
			error_code& ec) {
...
private:
	...
	udp_socket(io_service& ios) :
			m_socket(ios, udp::v4()), m_abort(false) {
	}

	udp_socket(io_service& ios, udp::endpoint bindto) :
			m_socket(ios, bindto), m_abort(false) {
	}
	...
	void on_receive(error_code const& ec, size_t bytes_transferred) {
		...
		if (ec == asio::error::operation_aborted) {
			log_debug("udp_socket::on_receive canceled, exiting: %s",
					ec.message().c_str());
			return;
		}
		...
		if (!ec)
			m_handler((char *) m_receive_buffer, bytes_transferred, m_sender);
		else
			log_debug("udp_socket::on_receive ignoring error: %s",
					ec.message().c_str());
