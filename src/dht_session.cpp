...
struct udp_socket_adaptor: UDPSocketInterface {
	udp_socket_adaptor(udp_socket* s) :
			m_socket(s), m_enabled(true) {
	}
	...
	void Send(const SockAddr& dest, cstr host, const byte *p, size_t len,
			uint32 flags = 0) {
		if (!m_enabled)
			return;
	...
	void Send(const SockAddr& dest, const byte *p, size_t len,
			uint32 flags = 0) {
		if (!m_enabled)
			return;
...
try {
	...
	log_error("failed to save DHT state to disk: (%d) %s", ec.value(),
			ec.message().c_str());
}
catch (std::exception& e) {
	log_error("failed to save DHT state to disk: %s", e.what());
}

void bdecode_buffer_with_hash(BencodedDict& dict, char const* buffer,
		int size) {
	...
	if (size >= 24 && memcmp(pos + 20, "hash", 4) == 0) {

		// there is a hash at the end of the file
		// verify it
		sha1_hash hash = sha1_fun((byte const*) buffer,
...
bool ed25519_verify(const unsigned char *signature,
	...
	return 0
			== crypto_sign_verify_detached(signature, message, message_len, key);
}
...
void map_upnp(int port, std::vector<upnp_mapping>& mappings) {
	...
	for (UPNPDev* device = devlist; device; device = device->pNext) {
		...
		char lanaddr[64];
		// 1 is connected IGD
		if (UPNP_GetValidIGD(device, &urls, &data, lanaddr, sizeof(lanaddr))
				== 1) {
			char port_str[30];
			snprintf(port_str, sizeof(port_str), "%d", port);
			int r = UPNP_AddPortMapping(urls.controlURL, data.first.servicetype,
					port_str, port_str, lanaddr, "BitTorrent Bleep", "UDP", "*",
					"0");
			...
			if (r != UPNPCOMMAND_SUCCESS) {
				...
				mappings.push_back(
						upnp_mapping(urls.controlURL, data.first.servicetype));
			}
		}
	}
}
...
void map_natpmp(int port, int time_to_live,
		scout::dht_session::run_state const& state, bool& is_natpmp_mapped) {
	bool is_quitting = state == scout::dht_session::QUITTING;
	if (is_quitting && !is_natpmp_mapped)
		return;
	...
	int r = sendnewportmappingrequest(&natpmp_state, NATPMP_PROTOCOL_UDP, port,
			port, time_to_live);
	...
	do {
		...
		if (r < 0 && r != NATPMP_TRYAGAIN) {
			...
			log_error("  errno=%d '%s'", sav_errno, strerror(sav_errno));
		}
	} while (r == NATPMP_TRYAGAIN);
	if (r >= 0)
		is_natpmp_mapped = !is_natpmp_mapped;
}
}
...
struct ip_change_observer_session: ip_change_observer {
	dht_session * m_ses;
	ip_change_observer_session(dht_session* c) :
			m_ses(c) {
	}
...
dht_session::dht_session() :
		m_socket(udp_socket::construct(m_ios)), m_dht_external_port(
				32768 + std::random_device()() % 16384), m_state(INITIAL), m_external_ip(
				&sha1_fun), m_dht_timer(m_ios), m_natpmp_timer(m_ios_worker), m_is_natpmp_mapped(
				false), m_dht_rate_limit(8000) {
	m_bootstrap_nodes.push_back(
			std::pair<std::string, int>("router.utorrent.com", 6881));
	m_bootstrap_nodes.push_back(
			std::pair<std::string, int>("router.bittorrent.com", 6881));
}
...
int dht_session::start() {
	...
	m_thread = std::move(
			std::thread(&dht_session::network_thread_fun, this,
					std::ref(promise)));
...
void dht_session::synchronize(secret_key_span shared_key,
		std::vector<entry> entries, entry_updated entry_cb,
...
void dht_session::put(list_token const& token,
...
auto new_end =
		std::remove_if(m_bootstrap_nodes.begin(), m_bootstrap_nodes.end(),
...
m_ios_worker.post(
		std::bind(&map_upnp, m_dht_external_port, std::ref(m_upnp_mappings)));

m_ios_worker.post(
		std::bind(&map_natpmp, m_dht_external_port, natpmp_interval * 3 / 2,
				std::cref(m_state), std::ref(m_is_natpmp_mapped)));
}
...
m_dht = create_dht(&socket_adaptor, &socket_adaptor, &save_dht_state,
		&load_dht_state, &m_external_ip);
...
while (!is_quitting()) {
	m_ios.run(ec);
	if (ec) {
		log_error("io_service::run: (%d) %s", ec.value(), ec.message().c_str());
		break;
	}
	m_ios.reset();
}
...
if (ec) {
	if (ec != boost::asio::error::operation_aborted) {
		log_error("natpmp timer failed: (%d) %s", ec.value(),
				ec.message().c_str());
	}
	return;
}
...
void dht_session::incoming_packet(char* buf, size_t len,
...
error_code const& ec = e.code();
log_error("error in incoming_packet: (%d) %s", ec.value(),
		ec.message().c_str());
}
catch (std::exception& e) {
